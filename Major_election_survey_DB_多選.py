...
# streamlit run Major_election_survey_DB.py
...
question_dict = {
    '投給候選人':{'type':'multiselect', 'required':True, 'options':['蔣萬安', '陳時中', '黃珊珊', '黃國昌', '其他']}, 
...
if tab1 == '投票':
    ...
    if st.button('送出'):
        ...
        new_data={}
        for key in question_dict.keys():
            if question_dict[key]['type'] == 'multiselect':
                new_data[key]=','.join(question_dict[key]['response'])
            else:
